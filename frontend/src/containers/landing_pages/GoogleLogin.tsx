...
class GoogleLogin extends React.Component<IProps, {}> {
  ...
  private async login(idToken: string) {
    ...
    api
      .post(
        `login`,
        { googleLogin: true, googleToken: idToken },
        {
          headers: {
  ...
  public componentDidMount() {
    ...
    (window as any).gapi.load("auth2", () => {
      (window as any).gapi.auth2
        .init({
          client_id:
            "1078865227946-n88q5b9jgmf5nolqppi1800e18ttsrfh.apps.googleusercontent.com"
        })
        .then(() => {
          const auth2 = (window as any).gapi.auth2.getAuthInstance();
          ...
          if (isSignedIn) {
            ...
            currentUserName =
              currentUser && currentUser.w3 ? currentUser.w3.ig : null;
  ...
  public render() {
    return (
      <button
        id="customBtn"
        className="custom-google-button  button small-button white-button is-centered is-flex"
      >
